...
<title>Danh sách hội đồng - Đăng ký đề tài</title>
</head>
<body>
	<div id="root">
		<!-- Header -->
							...
							<div class="topic_registration-detail">
								...
								<c:forEach var="item" items="${boards}">
									<div class="group_topic_registration-to-manage">
										<table>
											<tr>
												<th width="20%" class="highlight_content">${item.getBoardId()}</th>
												<th width="40%">${item.getBoardName()}</th>
												<th width="25%">${item.getNoMember()}</th>
												<th><a width="15%"
													href="<%=context%>/teacher/board/head/detail/?board_id=${item.getBoardId()}"
